...
class OrderValidationDeployTests(OrderBaseTests):
    def setUp(self):
        self.setup_order()
    ...
    def test_create_shortcut__with_latest_empty__something_is_copied_to_latest(self):
        self.order.validation_deployer.path_actions.create_shortcut_to_exe()
        self.assertTrue(self.os_module.path.exists(r'c:\xxx\order\uservalidations\latest\order.lnk'))

    def test_create_shortcut__with_shortcut_exists_in_target__copy_without_error(self):
        fake_destination_link = r'c:\xxx\order\uservalidations\latest\order.lnk'
    ...
    def test_create_shortcut__with_target_exists_in_candidates__extract_shortcut_target_works(self):
        # Arrange
        fake_target_path = r'c:\xxx\order\candidates\release-1.0.0\validation\order.exe'
        self.filesystem.create_file(fake_target_path)
        dest_shortcut_path = r'c:\xxx\order\uservalidations\latest\order.lnk'

        # Act
        self.order.validation_deployer.run()
    ...
    def test_copy_from_next__with_latest_dir_empty__copied_files_exists_in_latest(self):
        # Arrange
        validation_file = \
            r'c:\xxx\order\uservalidations\allversions\_next_release\validationfiles\validation_file.txt'
        self.filesystem.create_file(validation_file)
        self.file_builder.add_shortcut()

        # Act
        self.order.validation_deployer.copy_validation_files()

        # Assert
        copied_file = r'c:\xxx\order\uservalidations\latest\validationfiles\validation_file.txt'
    ...
    def test_copy_from_next__with_hotfix_candidate__files_copied_from_next_hotfix(self):
        # Arrange
        validation_file = \
            r'c:\xxx\order\uservalidations\allversions\_next_hotfix\validationfiles\validation_file.txt'
        self.filesystem.create_file(validation_file)
        self.order.branch_provider.candidate_branch = "hotfix-1.0.1"
        self.file_builder.add_shortcut()
    ...
    def test_copy_from_next__with_branch_and_shortcut_different__validation_files_copied_latest_to_archive(self):
        ...
        # Assert
        copied_file = r'c:\xxx\order\uservalidations\allversions\0.1.0\validationfiles\old_validation_file.txt'
        self.assertTrue(self.os_module.path.exists(copied_file))
    ...
    def test_copy_from_next__with_branch_and_shortcut_different__shortcut_copied_to_archive(self):
        ...
        # Assert
        copied_file = r'c:\xxx\order\uservalidations\allversions\0.1.0\order.lnk'
        self.assertTrue(self.os_module.path.exists(copied_file))
    ...
    def test_copy_from_next__with_branch_and_shortcut_different__sql_script_not_copied_to_archive(self):
        ...
        # Assert
        copied_file = r'c:\xxx\order\uservalidations\allversions\0.1.0\sqlupdates\script1.sql'
    ...
    def test_copy_from_next__with_branch_and_shortcut_different__file_deleted_in_latest(self):
        ...
        # Assert
        old_file = r'c:\xxx\order\uservalidations\latest\validationfiles\old_validation_file.txt'
    ...
    def test_copy_from_next__with_branch_and_shortcut_same__validation_files_not_copied_to_archive(self):
        ...
        # Assert
        copied_file = r'c:\xxx\order\uservalidations\allversions\1.0.0\validationfiles\old_validation_file.txt'
        self.assertFalse(self.os_module.path.exists(copied_file))
    ...
    def test_copy_from_next__with_file_in_archive__file_removed_in_archive(self):
        ...
        # Assert
        source_file = r'c:\xxx\order\uservalidations\allversions\1.0.0\validationfiles\validation_file.txt'
    ...
    def test_copy_from_next__with_file_in_archive__file_from_next_in_latest(self):
        ...
        # Assert
        copied_file = r'c:\xxx\order\uservalidations\latest\validationfiles\file_from_next.txt'
        self.assertTrue(self.os_module.path.exists(copied_file))
    ...
    def test_copy_from_next__ordinary_setup__only_one_catalog_and_shortcut_in_latest(self):
        ...
        # Assert
        latest = r'c:\xxx\order\uservalidations\latest'
        dir_objects = [o for o in self.order.os_service.listdir(latest)]
...
class FileSystemBuilder:
    ...
    def add_validation_file_in_archive(self, filename='validationfile.txt', contents=''):
        ...
        :return:
        """
        path = os.path.join(self.order.path_properties.archive_dir_validation_files, filename)
        self.filesystem.create_file(path, contents=contents)

    def get_contents(self, path):
        c = None
        with self.order.os_service.open(path, 'r') as f:
            c = f.read()
        return c
